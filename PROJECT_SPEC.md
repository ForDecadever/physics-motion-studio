...
- **编辑顺手**：选择、移动、旋转、对象缩放、吸附、图层、撤销/重做符合图形编辑软件习惯。
...
- 墙面吸附是独立开关并默认关闭；开启后，创建或移动物体会让碰撞外轮廓与最近地面相切。候选距离必须使用鼠标所控制的物体中心到最近地面点的真实欧氏距离，再扣除该方向的物体支撑半径；不得只按地面法向距离判断，也不得在沿切线远离端点时强制吸回。
- 物块吸附是独立开关并默认开启；新建或移动的小球、物块可与可见物块的真实旋转矩形边界相切。直边按面接触，角点按欧氏最近方向接触；浅层重叠只在 `12 px` 屏幕阈值内向外修正，深层重叠不得瞬移。
- 网格吸附先确定初始位置，已开启的墙面和物块候选再按修正距离最小者竞争；等距离时保持场景顺序。多选只由鼠标抓住的锚点物体求解并向整组应用同一位移，必须排除自身和全部同组对象。
- 移动、创建、控制点和锚点可吸附网格。
- 旋转默认按 `15°` 吸附。
...
- 图层和所有实体行均提供铅笔重命名按钮，双击名称也可进入编辑；名称去除首尾空格，最长 80 个字符，允许重名，空名称恢复原值。
...
- 当前场景格式为 `schemaVersion: 7`。`3 -> 4` 迁移为所有旧地面写入双面碰撞字段；`4 -> 5` 为旧 `GroundJointEntity` 写入自动过渡长度和未翻转方向；`5 -> 6` 为旧小球和物块写入 `rotationEnabled: true`；`6 -> 7` 为旧场景补充默认空白 `t-y` 坐标系。迁移不覆盖旧实体已有的数据，也不会尝试迁移此前未存入场景文件的运行采样。
...
- [x] 能用绳、杆、弹簧连接两个有效锚点。
- [x] 选择、移动、旋转、对象等比缩放、吸附、多选、删除和撤销/重做一致可靠。
- [x] 图层隐藏和锁定不会偷偷改变模拟结果。
