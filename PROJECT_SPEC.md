...
- [ ] 首次进入能看懂菜单、工具栏、画布、属性、图层和播放区的分工。
- [ ] 能创建直线、圆弧和贝塞尔地面，控制点编辑正常。
- [ ] 能创建质点、小球、物块和点电荷。
- [ ] 能创建矩形、圆形、多边形或无限范围的重力/电场/磁场。
- [ ] 能用绳、杆、弹簧连接两个有效锚点。
- [ ] 选择、移动、旋转、吸附、多选、删除和撤销/重做一致可靠。
- [ ] 图层隐藏和锁定不会偷偷改变模拟结果。
- [ ] 播放、暂停、单步、倍速和重置行为符合固定步长规则。
- [ ] 属性编辑显示正确单位，非法值不会进入场景。
- [ ] 至少可同时绘制两个实体的多个物理量时间曲线。
- [ ] 可以导出 CSV，并且表头含指标和单位。
- [ ] 保存后重新打开，实体、图层、参数和视图约定的数据不丢失。
- [ ] 物理标准验证场景全部在规定容差内通过。
- [ ] 达到或诚实记录第 13 节性能目标的测试结果。
- [ ] 主要操作可用键盘完成，错误信息能指导用户修正。
- [ ] 无默认遥测、无不必要网络请求、无动态执行存档内容。
