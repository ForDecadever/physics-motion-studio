...
一个采用专业图形编辑器布局的二维物理运动模拟工具。当前已完成 [PROJECT_SPEC.md](./PROJECT_SPEC.md) 中的阶段 4，形成可编辑、可模拟、可记录和可保存的第一版。

## 当前已经具备

- React + TypeScript + Vite 工程和 Photoshop 风格编辑布局；
- PixiJS 二维画布、自适应米制网格、平移、缩放、选择、移动、旋转与吸附；
- 创建质点、小球、物块、点电荷、直线地面、圆弧地面和贝塞尔地面；
- 绘制矩形、圆形、多边形或无限范围的匀强重力场、电场和磁场，支持区域叠加；
- 计算点电荷间库仑力，并可在场景属性中关闭或开启；
- 使用绳、杆或弹簧连接两个物体，编辑长度、刚度、阻尼和局部锚点；
...
- IndexedDB 自动草稿、意外刷新恢复，以及格式 1 到格式 2 的纯函数迁移；
